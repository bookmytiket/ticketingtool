...
import { Plus, Edit, Trash2, Search, Download, Upload, MoreHorizontal } from 'lucide-react'
...
export const Users = () => {
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      console.log('Initiating delete request for:', userId)
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50/80 backdrop-blur-sm">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Dept</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user._id || user.id} className="hover:bg-gradient-to-r hover:from-primary-50/50 hover:to-transparent transition-all duration-300 border-l-2 border-transparent hover:border-primary-500">
                      <td className="px-3 py-2 whitespace-nowrap">
                        <div className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-primary-100/80 backdrop-blur-sm text-primary-700 border border-primary-200">
                          {user.name}
                        </div>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-[11px] text-gray-900">{user.email}</td>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">
                        <Badge className="text-[10px] px-1.5 py-0" variant={user.role === 'admin' ? 'danger' : user.role === 'technician' ? 'info' : user.role === 'department-head' ? 'warning' : 'info'}>
                          {user.role}
                        </Badge>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <Badge className="text-[10px] px-1.5 py-0" variant={user.status === 'active' ? 'success' : 'warning'}>
                          {user.status}
                        </Badge>
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-[11px] text-gray-500">
                        {user.createdAt ? format(new Date(user.createdAt), 'MMM dd, yy') : 'N/A'}
                      </td>
                      <td className="px-3 py-2 whitespace-nowrap text-right text-[11px] font-medium relative">
                        <div className="flex items-center justify-end">
                          <div className="relative inline-block text-left">
                            <button
                              onClick={(e) => {
                                e.stopPropagation()
                                setActiveDropdownId(activeDropdownId === user._id ? null : user._id)
                              }}
                              className="text-gray-400 hover:text-primary-600 transition-colors p-1 rounded-md hover:bg-primary-50/50 backdrop-blur-sm"
                            >
                              <MoreHorizontal size={16} />
                            </button>

                            {activeDropdownId === user._id && (
                              <div className="absolute right-0 mt-1 w-24 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                                <div className="py-1">
                                  <button
                                    onClick={() => handleOpenModal(user)}
                                    className="flex items-center w-full px-3 py-1.5 text-[11px] text-primary-600 hover:bg-primary-50 transition-colors"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="flex items-center w-full px-3 py-1.5 text-[11px] text-red-600 hover:bg-red-50 transition-colors"
                                  >
